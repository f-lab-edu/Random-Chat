...
import com.example.ranchat.chatroom.controller.RanChatResponse;
...
public class ChatRoomService {
    ...
    public String ranChat(String userId) {
        ...
        // 상대가 나를 매칭했는지 확인
        if ("true".equals(redisService.isMatched(userId))) {
        ...
        if (otherUserId != null && !otherUserId.equals(userId)) {
            ...
            if ("true".equals(redisService.isMatched(otherUserId))) {
    ...
    private void makeMatchState(String userId, String otherUserId) {
        redisService.setMatchStatusTrue(userId);
        redisService.setMatchStatusTrue(otherUserId);
    }
